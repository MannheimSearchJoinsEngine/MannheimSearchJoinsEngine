...
public class ExperimentalDuplicateResolver {
	private Pipeline pipeline;

	public ExperimentalDuplicateResolver(Pipeline pipeline) {
	...
	private class DuplicateResolverThread implements Runnable {
		...
		private TableColumn mergeColumns(TableColumn leftColumn,
			...
			for (Entry<Integer, String> entry : leftColumn.getValues()
					.entrySet()) {
				...
				if (leftValue.equals(PipelineConfig.NULL_VALUE.toLowerCase())) {
					column.getValues().put(index, getNonNullValue(rightValues));
					continue;
				}
				if (getNonNullValue(rightValues).equals(
		...
		private String getNonNullValue(List<String> rightValues) {
			...
			for (String s : rightValues) {
				if (!s.equals(PipelineConfig.NULL_VALUE.toLowerCase())) {
					return s;
				}
			}
			return PipelineConfig.NULL_VALUE.toLowerCase();
		}
